...
import Model.MS2_01031_8without as PlaNet_C
importlib.reload(PlaNet_C)
net = PlaNet_C.Net()
net.load_state_dict(torch.load('Result/step5/PlaNet_Cwithout.pth'))   
...
for s in tqdm(range(int(len(data_ft))), desc="Loop level A: each case", position=0, leave=False):
    ...
    filename = 'Result/step7_2_without/'+str(s)
...
torch.save(AUC, "Result/step7_2_without/AUC")
torch.save([fpr, tpr, thresholds], "Result/step7_2_without/ROC")
torch.save(pred, "Result/step7_2_without/pred")
torch.save(t, "Result/step7_2_without/t")
