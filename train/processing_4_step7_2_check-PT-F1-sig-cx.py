...
import Model.MS2_01031_8sig as PlaNet_C
importlib.reload(PlaNet_C)
net = PlaNet_C.Net()
net.load_state_dict(torch.load('Result/step5/PlaNet_Ccheck-PT-F1-sig.pth'))   
...
for s in tqdm(range(int(len(data_ft))), desc="Loop level A: each case", position=0, leave=False):
    ...
    filename = 'Result/step7_2_check-PT-F1-sig-cx/'+str(s)
...
torch.save(AUC, "Result/step7_2_check-PT-F1-sig-cx/AUC")
torch.save([fpr, tpr, thresholds], "Result/step7_2_check-PT-F1-sig-cx/ROC")
torch.save(pred, "Result/step7_2_check-PT-F1-sig-cx/pred")
torch.save(t, "Result/step7_2_check-PT-F1-sig-cx/t")
